...
from ANNNI import vector_multiplication, update_matrices
...
def calculate_f(temp, L, k):
    ...
    Transfer_matrix = sla.LinearOperator((4**L, 4**L), matvec = lambda v: vector_multiplication(v, L, *matrices))
...
def calculate_f_array(temp, L, k):
    ...
    for i in range(temp.size):
        ...
        matrices = update_matrices(L, J, K, kappa, kappa_1, h0, h1, beta)
        Transfer_matrix = sla.LinearOperator((4**L, 4**L), matvec = lambda v: vector_multiplication(v, L, *matrices))
        f[i] = k*temp[i]*np.log(sla.eigs(Transfer_matrix, k=1, return_eigenvectors = False)[0])/2
    return f
