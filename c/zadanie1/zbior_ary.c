...
void zbior_push(zbior_ary *A, long long nowy_p, long long nowy_k) {
    if (A->uzyte == A->rozmiar) {
        long long *p_r = (long long *)realloc(A->p, sizeof(long long) * (size_t)((A->rozmiar) * 2));
        if (p_r == NULL) {
            exit(1);
        }

        long long *k_r = (long long *)realloc(A->k, sizeof(long long) * (size_t)((A->rozmiar) * 2));
        if (k_r == NULL) {
            exit(1);
        }
...
zbior_ary iloczyn(zbior_ary A, zbior_ary B) {
    long long a = 0, b = 0, nowy_p = 0, nowy_k = 0;
    zbior_ary C = nowy_zbior(A.rozmiar);
    if (A.uzyte == 0 || B.uzyte == 0) return C;
...
bool nalezy(zbior_ary A, long long x) {
    if (A.uzyte == 0) return false;
    long long r = modulo(x);
    long long lo = 0, hi = A.uzyte - 1;
    while (lo <= hi) {
        long long mid = lo + (hi - lo) / 2;
        long long rmid = modulo(A.p[mid]);
        if (rmid < r || (rmid == r && A.k[mid] < x)) {
            lo = mid + 1;
        } else if (rmid > r || (rmid == r && A.p[mid] > x)) {
            hi = mid - 1;
        } else {
            return (A.p[mid] <= x && x <= A.k[mid] && ((x - A.p[mid]) % q == 0));
        }
    }
    return false;
}
